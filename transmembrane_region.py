...
def get_uniprot_entry(accession, max_retries=3, attempt=0):
    """Fetch UniProt entry data using UniProt API with retry logic."""

    url = f"https://www.ebi.ac.uk/proteins/api/proteins/{accession}"
    print(f"requesting: {accession}")
    response = requests.get(url)
    try:
        response.raise_for_status()
    ...
    # Retry with exponential backoff, up to max_retries
        ...
        if attempt < max_retries:
            ...
            return get_uniprot_entry(accession, max_retries, attempt + 1)
        else:
            print(f"Max retries exceeded for {accession}")
            return None
...
if __name__ == "__main__":
    ...
    # Use the refactored longest_sub function (removes ~70 lines of duplicated code)
    longest_subs = longest_sub(pdb_ids)

    # Write results to file for standalone execution
    with open("longest_subs", "w") as f:
        for key, value in longest_subs.items():
            f.write(f"{key}: {value}\n")
