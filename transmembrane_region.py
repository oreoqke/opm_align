...
def graphQL_query(pdb_ids, logger):
    """
    ...
    if response.status_code == 200:
        ...
        logger.error(f"Failed to get data from RCSB GraphQL API. Status code: {response.status_code}")
    

def fetch_uniprot_entry(uniprot_id, logger):
    ...
    try:
        ...
        logger.error(f"Error occurred when fetching UniProt entry: {error}")
        logger.warning(f"Error occurred when fetching UniProt entry: {error}")
        # with open("error_log", "a") as f:
        #     f.write(f"Error occurred when fetching UniProt entry: {error}\n")
            
def parse_uniprot_entry(uniprot_xml, logger):
...
def is_membrane(subcellular_locations, keywords, logger):
...
def get_uniprot_entry(accession, logger):
    ...
    # To prevent the code from crashing retry the request in 5 seconds
        ...
        time.sleep(5)
        get_uniprot_entry(accession, logger)
...
def longest_sub(pdb_ids, logger):
    '''For a list of pdb_ids this function will find the subunit with the most transmembrane regions'''
    ...
    data = graphQL_query(pdb_ids, logger)['data']['entries']
    ...
    for key, value in pdb_uniprot.items():
        ...
        for i,entity in value.items():
            ...
            for uniprot in entity['uniprot']:                
                uniprot_data = get_uniprot_entry(uniprot, logger)
                # print(uniprot_data)
                ...
                if transmembrane_regions:
                    ...
                    uni_data = fetch_uniprot_entry(uniprot, logger)
                    subcellular_location, keywords = parse_uniprot_entry(uni_data, logger)
                    if is_membrane(subcellular_location, keywords, logger):
