...
def run_align(pdb_folder, output_dir, options, result_df, to_skip, target, ref):
    ...
    for result in re.finditer(pattern, output_splits):
        ...
        to_add.to_csv(f'{output_dir}/results.csv', mode='a', header=False, index=False)
        result_df = pd.concat([result_df, to_add], ignore_index=True)
    return result_df



for target in tqdm(target_list['PDB'], desc='\033[94m'+'Performing superposition using USalign'+ '\033[0m',disable=None, postfix='targets', leave = False):
# for target in target_list['PDB']:
    ...
    if exact_matching is True:
      ...
      for ref in tqdm(ref_list['PDB'], leave = False, disable=None,postfix='references'):
      # for ref in ref_list['PDB']:
          if (target,ref) in to_skip or target == ref: continue
          result_df = run_align(pdb_folder, output_dir, options, result_df, to_skip, target, ref)  
    print('\033[92m'+f'Finished superpositioning using USalign for {target} in {time.time() - target_time:.2f} seconds.'+'\033[0m')
    if not sys.stdout.isatty(): sys.stdout.flush()
